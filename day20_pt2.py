...
# To solve the second part, all that I had to do was to make k iterate 50 times and also add the bit at the bottom
for k in range(50):
    nextLines = []
    for i in range(len(lines)):
        mini = []
    ...
    # This adds a layer of # or . depending on what the parity of k is
    if k % 2 == 0:
        for i in range(len(lines)):
            lines[i].insert(0,'#')
            lines[i].append('#')
